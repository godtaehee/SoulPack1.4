...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("정말 밖으로 나가고 싶으신가요? 나갔다가 다시들어올경우 처음부터 다시진행해야합니다.");
		} else if (status == 1) {
			cm.sendNextPrev("정말 밖으로 나가고 싶으신가요? 나갔다가 다시들어올경우 처음부터 다시진행해야합니다.")
		} else if (status == 2) {
			if (cm.getMeso() < 0) {
				cm.sendOk("　")
				cm.dispose();
			} else {
				cm.sendYesNo("정말 밖으로 나가고 싶으신가요? 나갔다가 다시들어올경우 처음부터 다시진행해야합니다.");
			}
		} else if (status == 3) {
			cm.warp(103000100, 0);
			cm.dispose();
