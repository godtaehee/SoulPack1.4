...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		cm.sendYesNo("좋았어! 잘생각했네. 가격은 그다지 비싸지 않아. 내가 큰 맘 먹고 #r대폭할인#k을 시작했기 때문이지. #b친구 목록 5명을 추가 하는 데 24만 메소#k라네. 물론 낱개로는 팔지 않는다네. 한 번 사두기만 하면 영원히 목록이 늘어나는 셈이니까 친구 목록이 부족한 사람이라면 해보는 것도 나쁘지 않아. 어때? 5만 메소 내겠나?");
	} else if (status == 2) {
		var capacity = cm.getPlayer().getBuddylist().getCapacity();
		if (capacity >= 50 || cm.getMeso() < 240000){
			cm.sendNext("자네... #b24만 메소#k는 확실히 가지고 있는 건가? 만일 그렇다면 친구 목록이 최대로 늘어나 있는건 아닌지 확인해 주게. 아무리 돈을 낸다 하더라도 친구 목록은 기본 개수를 포함해서 #b50개 이상#k 늘어날 수 없다네.");
			cm.dispose();
		} else {
