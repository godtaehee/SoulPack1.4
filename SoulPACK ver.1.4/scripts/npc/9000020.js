...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			cm.sendYesNo("전 소울서버에 스킬을 마스터해주는 스피넬이라고 해요, 모든스킬을 마스터해주죠.\r\n스킬을 마스터하고 싶으신가요? 스킬마스터후에는 약3초정두있다가, #r로그아웃#k을 하시면됩니다!\r\n");
		} else if (status == 1) { 
            cm.sendNext("다음에 또 와주세요 ^^ by.소울 "); 
        } else if (status == 2) { 
            cm.clearSkills(); 
            ...
            cm.teachSkill(5220011,20,20); // Ended CORSAIR Skills   // Fully Ended Pirate Skills For V62  
            cm.dispose(); 
        } 
    } 
}  
