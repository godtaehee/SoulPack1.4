...
var playerStatus;
var chatState;
var questions = Array("문제입니다. 전사가 전직하려면 필요한 레벨에 수만큼 쿠폰을 모아오세요.",
			"문제입니다. 전사가 전직하려면 필요한 힘에 수만큼 쿠폰을 모아오세요.",
			"문제입니다. 마법사가 전직하려면 필요한 인트에 수만큼 쿠폰을 모아오세요.",
			"문제입니다. 궁수가 전직하려면 필요한 덱스에 수만큼 쿠폰을 모아오세요.",
			"문제입니다. 도적이 전직하려면 필요한 덱스에 수만큼 쿠폰을 모아오세요.",
			"문제입니다. 2차전직을하려면 필요한 레벨에 수만큼 쿠폰을 모아오세요.");
		...
		if (curMap == 1) { // First Stage.
			if (playerStatus) { // party leader
				if (status == 0) {
					...
					if (preamble == null) {
						cm.sendNext("첫번째 스테이지에 오신것을 환영합니다. 첫번째스테이지에서는 리게이터를 잡아서 쿠폰을 획득하여 저에게 갖다주어야 하는미션입니다. 제가 문제를내면 그문제에 답만큼에 쿠폰에 갯수를맞추어 저에게 오시면 통행증을 드리겠습니다. 파티장이 통행증을 모두 모아오면 다음스테이지로 넘어가는 문이열리게됩니다.");
						...
						cm.dispose();
                        			...
                        			if (complete != null) {
                        				cm.sendNext("다음스테이지로 넘어가주세요. 포탈이 이미 열려있습니다.");
                        				cm.dispose();
                        			}
                        			else {
							...
							if (!passes) {
								cm.sendNext("죄송하지만 통행증이 부족합니다. 통행증을 파티장을제외한 파티를하고있는 모든유저에게 걷어오셔서 저에게 다시말을 걸어주세요.");
								cm.dispose();
							}
							else {
								cm.sendNext("첫번째 스테에지를 클리어하셧습니다! 포탈을 열어드리겠습니다. 다음스테이지로 넘어가주세요.");
								clear(1,eim,cm);
				...
				if (status == 0 && preamble == null) {
					...
					cm.sendNext("정답이아닙니다. 문제에 정답만큼에 쿠폰수를 가져오세요.");
						...
						if (!qcorr) { // not too many
							...
							if (qcorr) { // just right
								cm.sendNext("정답입니다! 통행증을 파티장에게 넘겨주세요.");
						...
						if (!enough) {
							cm.sendNext("정답이 아닙니다. 제가낸 문제에 답 수만큼에 쿠폰을 모아서 말을걸어주세요.");
						}
						cm.dispose();
					}
				}
			...
			if (stage5done == null) {
				if (playerStatus) { // Leader
					...
					if (passes) {
						...
						cm.sendNext("마지막 스테이지입니다. 밑에있는 킹슬라임과 커즈아이,주니어네키들을 잡아서 통행증 10개를 모아오세요.");
					}
					cm.dispose();
				}
				else { // Members
					cm.sendNext("마지막 스테이지입니다. 밑에있는 킹슬라임과 커즈아이,주니어네키들을 잡아서 통행증 10개를 모아오세요. 통행증은 파티장에게 갖다주셔야합니다.");
					cm.dispose();
				}
			}
			else { // give rewards and warp out
				if (status == 0) {
					cm.sendNext("보상을 받으시려면 기타창,소비창,장비창중 1개이상에 여유 공간이필요합니다. 공간이 없으신분들은 여유공간 1개씩을 만들어주셔야합니다.");
				}
				if (status == 1) {
					getPrize(eim,cm);
					cm.dispose();
