...
var status = 0;
var maps = Array(104000000, 101000000, 100000000, 103000000);
var mapNames = Array("리스항구", "엘리니아", "헤네시스", "커닝시티");
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("안녕하세요~! 페리온 중형택시입니다. 다른 마을로 안전하고 빠르게 이동하고 싶으신가요? 그렇다면 저희 택시를 이용해 보세요. 싼 가격으로 원하시는 곳까지 친절하게 모셔다 드리고 있습니다.");
		} else if (status == 1) {
