...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status >= 2 && mode == 0) {
			cm.sendOk("해피빌엔 왜맨날 눈만내리는걸까? 그이유를알고싶어...");
			cm.dispose();
			return;
		}
		if (mode == 1)
			...
			status--;
		if (status == 0) {
				cm.sendNext("안녕? 난 산타고양이야. 내가지금 선물을 포장중인데, 선물상자가 모자라... 우선 파란색,노란색,초록색 선물상자가 필요하거든?\r\n파란색 선물상자는 슬리피우드에 #b믹스골렘#k에게 드롭하고\r\n노란색 선물상자는 페리온의 #b레드드레이크#k에게 드롭하고\r\n초록색 선물상자는 엘리니아의 #b멜러디#k에게 드롭가능해\r\n부탁인데 모두 25개씩 모아다줄수있지? 부탁할게...!");
			}
		else if (status == 1) {
			if (cm.getLevel() >= 70 && cm.haveItem(4031504, 25) && cm.haveItem(4031504, 25) && cm.haveItem(4031506, 25)) {
			cm.sendOk("벌써 모아왔구나? 고마워! 덕분에 선물포장을 빨리하게되었어!");
			} else {
			cm.sendOk("레벨이 70미만이거나, 선물상자가 모자라, 레벨이 70미만일경우 #b클리프#k할아버지 에게 가봐! #b클리프#k할아버지도 선물상자때문에 고민이시래... 레벨이 10미만일경우 #b루돌프#k에게 가봐! #b루돌프#k의 의뢰를들어주면 루돌프의코라는 신기한아이템을 준다던데!");
			cm.dispose();
			}
		}
		else if (status == 2) {
			cm.gainItem(4031504, -25);
			cm.gainItem(4031505, -25);
			cm.gainItem(4031506, -25);
			cm.gainMeso(5000);
			cm.gainExp(50000);
			...
			else if (itemchance >= 997) { 
			cm.gainMeso(20000);
			}
			else if (itemchance >= 998) { 
			cm.gainExp(60000);
			}
			else if (itemchance >= 999) { 
			cm.gainMeso(15000)
			}
			else if (itemchance >= 1000) { 
			cm.gainExp(50000);
			}
