...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status >= 2 && mode == 0) {
			cm.sendOk("ÅÚ¹Ì ÅÚ¹Ì Å×Å×Å×Å×Å×ÅÚ¹Ì!");
			cm.dispose();
			return;
		}
		if (mode == 1)
			...
			status--;
		if (status == 0) {
				cm.sendNext("³­ GM¿¤ÀÌ¸¸µç °³»þÆùÀÌ¾ß.\r\n³­ 1¾ïÀ» ÁÖ¸é ÅÛÀ» Áà~!\r\nÀÌ·¸°Ô »ý±ä È¥¾ËµµÁØ´Ù!: #v2041200#");
			}
		else if (status == 1) {
			if (cm.havemeso(100000000)) {
			cm.sendYesNo("1¾ïÀ» ½á¼­ ¾ÆÀÌÅÛ°ú ¹Ù²Ù°Ú´Ï?");
			}
			else if (!cm.havemeso(100000000)) {
			cm.sendOk("½Ã¸£¸é ÇÏÁö¸»´ø°¡ ,®X#k.");
			cm.dispose();
			}
		}
		else if (status == 2) {
