/*
Growlie HPQ Map 910010000
Coded by Kars & gachmaker
Cake itemid 4001101
*/
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
				cm.sendNext("어흥,, 토리를 살리고싶지??");
			}
		else if (status == 1) {
			if (cm.haveItem(4001101)) {
			cm.sendOk("어흥,,. #v04001101# 너무고마워 난 사실 채식주의자거든ㅋㅋㅋ");
			}
			else if (!cm.haveItem(4001101)) {
			cm.sendOk("#v04001101# #b한개를 빨리가져와#k");
			cm.dispose();
			}
		}
		else if (status == 2) {
			...
			cm.warp(910010100, 0);
			cm.sendOk("Muhahah u get NOTHING!"); // chance 198 caused memory leaks
			cm.gainExp(1600);
			}
