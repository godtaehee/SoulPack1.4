		...
		if (status == 0) {
			...
			if ((cm.getJob().equals(MapleJob.FIGHTER) ||
				...
				if (cm.getJob().equals(MapleJob.FIGHTER)) {
					...
					cm.getChar().gainAp(5);
					cm.sendOk("자넨 #b크루세이더#k가 되었다네. 3차직업을 가진만큼 가치있는생활을하시게나.");
					cm.dispose();
				} else if (cm.getJob().equals(MapleJob.SPEARMAN)) {
					cm.changeJob(MapleJob.DRAGONKNIGHT);
					cm.getChar().gainAp(5);
					cm.sendOk("자넨 #b드래곤나이트#k가 되었다네. 3차직업을 가진만큼 가치있는생활을하시게나.");
					cm.dispose();
				} else if (cm.getJob().equals(MapleJob.PAGE)) {
					cm.changeJob(MapleJob.WHITEKNIGHT);
					cm.getChar().gainAp(5);
					cm.sendOk("자넨 #b화이트나이트#k가 되었다네. 3차직업을 가진만큼 가치있는생활을하시게나.");
					cm.dispose();
			} else {
