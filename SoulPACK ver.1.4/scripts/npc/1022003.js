...
function action(mode, type, selection) {
	if (mode == 1)
		...
		cm.dispose();
	if (status == 0 && mode == 1) {
		var selStr = "안녕? 난 썬더라고해, 지금은 비록 은퇴한 전사지만 주먹펴고일어서님의 개시를 받고 아이템 제작에 힘써주고 있어. 무엇을 할꺼니?#b"
		var options = new Array("광석의원석으로 재련석 만들기","보석의원석으로 재련석 만들기","헬멧 업그레이드","방패 업그레이드");
		...
		if (selectedType == 0){ //mineral refine
			var selStr = "어떤 광석의원석으로 재련석을 만들거니?#b";
			...
			var selStr = "어떤 보석의원석으로 재련석을 만들거니?#b";
			...
			var selStr = "헬멧? 내전공이지! 어떤헬멧을 업그레이드 할거니?#b";
			...
			var selStr = "방패? 내전공이지! 어떤방패를 업그레이드 할거니?#b";
		...
		prompt += " 를 요청했지? 다음과 같은 재료가 필요해.#b";
