...
var msg = new Array("that cures the state of being sealed and cursed","that cures all",", possessing magical power, that is used for high-quality skills",", possessing the power of summoningm that is used for high-quality skills");
...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		selected = selection;
		cm.sendGetNumber("Is #b#t"+item[selected]+" really the item that you need? It's the item "+msg[selected]+". It may not be the easiest item to acquire, but I'll give you a good deal on it. It'll cost you #b"+cost[selected]+" mesos#k per item. How many would you like to purchase?", 0, 1, 100);
	}
	else if(status == 2) {
		amount = selection;
		...
		cm.sendYesNo("Are you sure you want to buy #r"+amount+" #t"+item[selected]+"(s)#k? It'll cost you "+cost[selected]+" mesos per #t"+item[selected]+", which will cost you #r"+totalcost+"#k mesos total.");
	} else if(status == 3) {
		...
		cm.sendMext("Thank you. If you ever find yourself needing items down the road, make sure to drop by here. I may have gotten old over the years, but I can still make magic items with ease.");
