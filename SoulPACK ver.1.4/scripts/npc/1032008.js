/*
	Cherry [1032008]
	엘리니아 매표소, 티켓을가지고있으면 오르비스 항해중 맵으로 이동.
	MADE BY 소희 (jantnic)
*/
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("어서오세요. 매표소안내원 조엘에게 티켓을 사시고온거 맞으시죠? 오르비스행 티켓이있어야 오르비스로 가실수있다는거 잊지마세요. 오르비스행 티켓은 5,000메소에 팔고있죠! 안사오셧다면 얼른사오세요!");
		} else if (status == 1) {
			...
			status=1;
				...
				if (selection == 0){ 
				        if (cm.haveItem(4031045)){
				        	cm.sendNext("오시리아대륙, 오르비스로 출발합니다! 배를타신후 정확히5분후에 오르비스에 도착합니다! 무시무시한 크림슨발록도 나타나니 조심하시기 바랍니다!");
				        	cm.gainItem(4031045,-1);
					status = 9;
				         }else{
					cm.sendOk("오르비스행 티켓을 가지고있지않네요. 티켓은 매표소안내원 조엘에게 구입할수있답니다.");
					cm.dispose();
					return;
				         }		
				} 
