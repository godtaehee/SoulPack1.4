/*
	Platform Usher (2012006)
	오르비스 매표소, 티켓을가지고있으면 그티켓또는 원하는마을 정거장통로로 이동
	MADE BY 소희 (jantnic)
*/
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("어서오세요. 매표소안내원 아가사에게 티켓을 사시고온거 맞으시죠? 사오시지않으셧다면 지금 당장 원하는곳 티켓을 사오시기바랍니다. 티켓이있어야 정거장통로로 이동시켜드립니다. 혹시라도 잘못들어가셧다면 표를 재구입하신뒤 다시저에게 말을걸어주시면 원하는곳으로 보내드릴수있습니다, 티켓은 반드시 재구입해야합니다.");
		} else if (status == 1) {
			cm.sendSimple("어디로 가시겠습니까?\r\n#L0#엘리니아행 [기본]#l\r\n#L1#엘리니아행 [일반]#l\r\n#L2#루디브리엄행 [기본]#l\r\n#L3#루디브리엄행 [일반]#l\r\n#L5#리프레행 [기본]#l\r\n#L6#리프레행 [일반]#l\r\n#L7#아리안트행 [기본]#l\r\n#L8#아리안트행 [일반]#l\r\n#L4#무릉행#l");
			status=1;
		} else if (status == 2) {
				if (selection == 0){ 
				        if (cm.haveItem(4031046)){
				...
				if (selection == 4){
					  ...
					  cm.sendOk("무릉으로 가시려면 최소한 1 메소는 가지고있으셔야합니다.");
					  cm.dispose();
					  return;
				          }
				}	
