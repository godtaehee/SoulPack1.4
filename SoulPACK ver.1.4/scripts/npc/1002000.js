/*
	...
	! 이 스크립트는 무한배포 되었으며 재배포가 가능합니다.
	! 이 스크립트는 노리팩 제작에 사용 되었습니다.
...
var mapNames = Array("페리온", "엘리니아", "헤네시스", "커닝시티", "노틸러스 선착장");
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status >= 27 && mode == 0) {
			...
			cm.dispose();
			return;
		} else if (status <= 3 && mode == 0 || status == 26 && mode == 0 || status == 6 && mode == 1 || status == 9 && mode == 1 || status == 12 && mode == 1 || status == 15 && mode == 1 || status == 18 && mode == 1 || status == 21 && mode == 1) {
			cm.dispose();
			return;
		}
		if (mode == 1)
		...
		if (status == 0) {
			...
			if (selection == 0) {
				...
				if (cm.getJob().equals(net.sf.odinms.client.MapleJob.BEGINNER)) {
					...
					var selStr = "넌 초보자가 아니로군? 그렇다면 요금을 전부 다 받아야 겠는걸? 자, 어떤 마을로 가보고 싶어?#b";
					for (var i = 0; i < maps.length; i++) {
			...
			if (cm.getJob().equals(net.sf.odinms.client.MapleJob.BEGINNER)) {
				cm.sendYesNo("이곳에서 더 이상 볼일이 없는 모양이로군. 정말로 #b" + mapNames[selection] + "#k 마을로 이동하고 싶은거야? #b" + costBeginner[selection] + " 메소#k만 내면 보내주지. 어때?");
			} else {
				cm.sendYesNo("이곳에서 더 이상 볼일이 없는 모양이로군. 정말로 #b" + mapNames[selection] + "#k 마을로 이동하고 싶은거야? #b" + cost[selection] + " 메소#k만 내면 보내주지. 어때?");
			}
		} else if (status == 28) {
			if (cm.getJob().equals(net.sf.odinms.client.MapleJob.BEGINNER)) {
