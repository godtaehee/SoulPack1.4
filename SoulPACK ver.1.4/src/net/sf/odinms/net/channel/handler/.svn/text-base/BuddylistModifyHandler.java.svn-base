	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		if (mode == 1) { // add
			...
			if (ble != null && !ble.isVisible()) {
				c.getSession().write(MaplePacketCreator.serverNotice(1, "The buddy is already registered."));
			} else if (buddylist.isFull()) {
				c.getSession().write(MaplePacketCreator.serverNotice(1, "Your buddy list is full."));
			} else {
				try {
					...
					if (charWithId != null) {
						...
						if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
							c.getSession().write(MaplePacketCreator.serverNotice(1, "The user's buddy list is full."));
						} else {
						...
						c.getSession().write(MaplePacketCreator.serverNotice(1, "That character is not registered."));
					}
