...
public class MapleStatEffect implements Serializable {
	...
	private static MapleStatEffect loadFromData(MapleData source, int sourceid, boolean skill, boolean overTime) {
		...
		if (skill) { // hack because we can't get from the datafile...
			switch (sourceid) {
				...
				case 2211005:
				case 5101006:
				case 5121009: // speed infusion - shouldn't actually be here
				case 5201003:
				case 5221010: // speed infusion - shouldn't actually be here
	...
	private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
		...
		if (isTimeLeap()) {
			for (PlayerCoolDownValueHolder i: applyto.getAllCooldowns()) {
				if (i.skillId != 5121010) applyto.removeCooldown(i.skillId);
			}
		}
		return true;
	}
	...
	private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
		...
		/*if (isMonsterRiding()) { // Old way, fixed by Bassoe
			int ridingLevel = 0;
			IItem mount = applyfrom.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -18);
			IItem weapon = applyfrom.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -11);
			if (sourceid != 5221006 && mount != null) {
				ridingLevel = mount.getItemId();
			} else if (sourceid == 5221006 && MapleItemInformationProvider.getInstance().isGun(weapon.getItemId())) {
				ridingLevel = 1932000;
			}
 ...
 */
		if (isMonsterRiding()) { // New way, Fixed by Bassoe (you dont need a gun! only to attack in the boat)
			int ridingLevel = 0;
			IItem mount = applyfrom.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -18);
			if (mount != null) {
				ridingLevel = mount.getItemId();
		...
		if (localstatups.size() > 0) {
			...
			log.warn(MapleClient.getLogMessage(applyto, "\r\nApplying empty statups (skill {}, id {})", skill, sourceid));
		}
		if (isDs()) {
