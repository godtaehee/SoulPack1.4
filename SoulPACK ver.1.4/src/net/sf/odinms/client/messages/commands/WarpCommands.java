...
public class WarpCommands implements Command {
	...
	public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception,
		...
		if (splitted[0].equals("!warp")) {
			...
			if (splitted.length > 1) { 
				MaplePacket packet = MaplePacketCreator.serverNotice(6, "[" + c.getPlayer().getName() + "] " + StringUtil.Korconv(splitted, 1));
				try {
	...
	public CommandDefinition[] getDefinition() {
		return new CommandDefinition[] {
			...
			new CommandDefinition("map", "mapid", "Warps you to the given mapid (use /m instead)", 100),
                        new CommandDefinition("exprate", "rate", "Sets the experience rate.", 100),
                        ...
                        new CommandDefinition("mesorate", "rate", "Changes the meso drop rate", 100),
		};
	}
